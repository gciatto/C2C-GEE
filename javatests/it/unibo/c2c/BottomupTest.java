...
public class BottomupTest {
  ...
  private static final String EXPECTED_FILE_FILTERED_INTERPOLATED_REGROWTH =
      "output-regrowth-negonly-interpolate.csv";

  private static Csv inputs,
      expectedDefault,
      expectedRevert,
      expectedFilter,
      expectedFilteredRegrowth;
  ...
  public static void loadFiles() {
    ...
    expectedFilteredRegrowth =
        Csv.vertical(BottomupTest.class.getResourceAsStream(EXPECTED_FILE_FILTERED_REGROWTH));
    expectedFilteredInterpolatedRegrowth =
        Csv.vertical(
            BottomupTest.class.getResourceAsStream(EXPECTED_FILE_FILTERED_INTERPOLATED_REGROWTH));
  }
  ...
  private void assertEquals(double actual, double expected) {
    Assert.assertEquals(
        "Failed equality assertion in %s, row with ID %d: %s != %s"
            .formatted(testName.getMethodName(), lastID, expected, actual),
        expected,
        actual,
        1e-9);
  }
