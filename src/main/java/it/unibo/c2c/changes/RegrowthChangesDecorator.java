...
record RegrowthChangesDecorator(Changes changes, DoubleList nextDates, DoubleList nextValues)
  ...
  public AllChanges withPost(double postMagnitude, double postDuration) {
    return new AllChangesDecorator(
        new RegrowthChangesDecorator(
            changes.withPost(postMagnitude, postDuration), nextDates, nextValues));
  }
}
